            ...
            request.length == 0 ? "" : <DataTable columns={columns} data={request} theme={'tableTheme'} expandableRows expandOnRowClicked expandableRowsComponent={
                (row) => {
                    return(
                        ...
                        <button onClick={(e) => {updateStatus('Rejected', row.data.id, row.data.requestType, row.data.fundAmount, row.data.employeeId)}}>Reject</button>
                        </div>)
            }
            }/>
        }
...
export default AccountingViewRequest
